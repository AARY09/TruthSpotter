...
function checkInitialized(req, res, next) {
    if (!isInitialized) {
...
app.post('/verify-claim', checkInitialized, verifyLimiter, async (req, res) => {
    try {
        ...
        const result = await detector.verifyClaim(claim.trim());
...
app.get("/verify-stream", checkInitialized, async (req, res) => {
    ...
    const context = {
        ...
        userName: req.user?.email || req.headers['x-user-email'] || undefined,
        conversationId: req.query.conversationId || undefined,
        requestId: crypto.randomUUID(),
        timestamp: new Date().toISOString(),
    };
    const orchestrator = new agent_orchestrator_1.AgentOrchestrator(detector, (msg) => send("step", msg), context);
    try {
...
app.post('/verify-claim-agentic', checkInitialized, verifyLimiter, async (req, res) => {
    try {
        ...
        const startTime = Date.now();
        const orchestrator = new agent_orchestrator_1.AgentOrchestrator(detector, undefined, context);
...
app.get('/news', async (req, res) => {
    try {
        ...
        const normalized = (articles || []).slice(0, limit).map((a) => ({
...
async function startServer() {
    try {
        ...
        app.listen(PORT, () => {
            ...
            if (!isInitialized)
                console.log('⚠️ Running in degraded mode: detector not initialized');
