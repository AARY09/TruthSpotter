...
export const MAX_QUERY_WORDS = 15;
...
export function enforceQueryWordLimit(
  query: string,
...
function fallbackPlan(claim: string): OptimizedSearchPlan {
  ...
  const primary = enforceQueryWordLimit(
    words.length > 10 ? `${words.slice(0, 10).join(' ')}${temporal}` : `${trimmed}${temporal}`
  );
  const factCheck = enforceQueryWordLimit(
    `${words.slice(0, 8).join(' ')} fact check verification`
  );
 ...
 */
export async function optimizeSearchQueries(claim: string): Promise<OptimizedSearchPlan> {
